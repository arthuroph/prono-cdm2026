...
/**
 * Parse "HH:MM UTC±N" → minutes depuis minuit UTC.
 * Ex : "13:00 UTC-6" → 13*60 - (-6*60) = 780 + 360 = 1140 min UTC = 19:00 UTC
 */
function parseKickoffUTC(timeStr) {
  ...
  return h * 60 + min - offset * 60; // minutes depuis minuit UTC
}
  ...
  try {
    ...
    for (const m of json.matches || []) {
      ...
      scores.push({
        ...
        score1: ft ? ft[0] : null,
        score2: ft ? ft[1] : null,
        kickoffUTC: parseKickoffUTC(m.time)
      });
    }
...
function getMergedMatches() {
  ...
  return baseMatches.map(m => {
    ...
    if (m.group) {
      ...
      if (live) {
        kickoffUTC = live.kickoffUTC ?? null;
        if (live.score1 != null) { score1 = live.score1; score2 = live.score2; }
      }
    } else {
    ...
    const merged = { ...m, score1, score2, kickoffUTC };
