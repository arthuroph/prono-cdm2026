 ...
 * @param {number|null} score1
 * @param {number|null} score2
 * @param {number|null} kickoffUTC  minutes depuis minuit UTC (ex: 1140 = 19:00 UTC)
 */
function getMatchStatus(dateStr, score1, score2, kickoffUTC) {
  ...
  if (dateStr > todayUTC) return 'a_venir';
  if (dateStr < todayUTC) return 'en_cours'; // date passée sans score

  // Aujourd'hui : utilise l'heure de coup d'envoi si disponible
  if (kickoffUTC == null) return 'en_cours';

  const nowMins = now.getUTCHours() * 60 + now.getUTCMinutes();
  if (nowMins < kickoffUTC) return 'a_venir';   // pas encore commencé
  return 'en_cours';                              // coup d'envoi passé, pas de score final
}
...
function computeParticipantScore(participant, allMatches, standings, best3rd, resolvedBracket, config) {
  ...
  for (const match of allMatches.filter(m => m.group)) {
    ...
    details.push({
      ...
      matchStatus: getMatchStatus(match.date, match.score1, match.score2, match.kickoffUTC ?? null),
      ...res
    });
  }
  ...
  for (const match of allMatches.filter(m => m.group == null)) {
    ...
    details.push({
      ...
      matchStatus: getMatchStatus(match.date, real?.score1 ?? null, real?.score2 ?? null, match.kickoffUTC ?? null),
      ...res
    });
  }
